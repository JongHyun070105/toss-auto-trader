...
def is_entry_gap_candidate(
    raw_gap: float,
    *,
    minimum_gap: float = MIN_RAW_ENTRY_GAP,
    maximum_gap: float = MAX_RAW_ENTRY_GAP,
) -> bool:
    """Return True when a raw gap is inside the live entry interval."""
    return minimum_gap <= raw_gap <= maximum_gap
