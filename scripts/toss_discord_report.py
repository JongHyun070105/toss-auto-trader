...
def sell_report(date: str | None = None) -> str:
    ...
    if buy and buy.get("order"):
        bo = buy["order"]
        lines.append(f"- 오전 매수: {bo['name']} {bo['qty']}주 @ {money(bo['expected_price'])}, 배정 {money(bo['amount'])}, 주문ID {buy.get('order_id') or '확인 필요'}")
    elif buy:
    ...
    if s.get("orders"):
        ...
        for o in s["orders"]:
            ...
            lines.append(f"  - {o['name']} {o['qty']}주 / 예상단가 {money(o.get('expected_price'))} / 예상금액 {money(o.get('expected_amount'))} / {status} / 주문ID {o.get('order_id') or '확인 필요'}")
        if buy and buy.get("order") and s["orders"]:
            bo = buy["order"]
            ...
            if first_sell.get("expected_price") and bo.get("expected_price"):
                ...
                lines.append(f"- 추정 당일 손익: {money(pnl)} ({pct(ret)}) — 실제 체결가는 Toss 체결내역으로 확정 필요")
            else:
                lines.append("- 추정 당일 손익: 매도 체결가/예상가 확인 필요")
    else:
...
def status_report() -> str:
    return "\n".join(
        [
            ...
            f"- 시간: {now_text()}",
            "- macOS crontab에서 주문/보고/종가/DB 업데이트를 직접 실행하도록 구성 예정/확인",
