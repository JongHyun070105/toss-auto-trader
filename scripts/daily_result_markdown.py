...
def daily_result_from_parsed(
    ...
    return DailyResult(
        ...
        return_pct=return_pct(entry, exit_price),
        note=note,
    )


def load_daily_result(date: str) -> DailyResult:
    ...
    return daily_result_from_parsed(date, buy, monitor, sell)
...
def main() -> int:
    ...
    result = load_daily_result(args.date)
    if args.print_only:
